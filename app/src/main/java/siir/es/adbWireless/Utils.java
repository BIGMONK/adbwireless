...
public class Utils {
    ...
    public static void WiFidialog(final Context context) {
        ...
        builder.setMessage(context.getString(R.string.no_wifi)).setCancelable(true)
                .setPositiveButton(context.getString(R.string.button_exit), new DialogInterface
                        .OnClickListener() {
                    public void onClick(DialogInterface dialog, int id) {
                        System.exit(0);
                    }
                }).setNegativeButton(R.string.button_activate_wifi, new DialogInterface
                .OnClickListener() {
            public void onClick(DialogInterface dialog, int id) {
    ...
    public static boolean adbStart(Context context) {
        try {
            ...
            if (Utils.prefsScreenOn(context)) {
                final PowerManager pm = (PowerManager) context.getSystemService(Context
                        .POWER_SERVICE);
                mWakeLock = pm.newWakeLock(PowerManager.SCREEN_BRIGHT_WAKE_LOCK, context.getClass
                        ().getName());
                mWakeLock.acquire();
            }

            if (Utils.prefsNoti(context)) {
                Utils.showNotification(context, R.drawable.ic_stat_adbwireless, context.getString
                        (R.string.noti_text) + " " + Utils.getWifiIp(context));
            }
        } catch (Exception e) {
            return false;
        }
    ...
    public static void autoConnect(Context context, String mode) {
        ...
        String urlRequest = "http://" + autoConIP + ":" + autoConPort + "/" + mode + "/" + Utils
                .getWifiIp(context);
    ...
    public static void showNotification(Context context, int icon, String text) {
        ...
        PendingIntent mContentIntent = PendingIntent.getActivity(context, 0, notifyIntent, 0);

        if (Build.VERSION.SDK_INT < 16) {
            notifyDetails = new Notification(icon, text, System.currentTimeMillis());
            Class clazz = notifyDetails.getClass();
            try {
                Method m2 = clazz.getDeclaredMethod("setLatestEventInfo", Context.class,
                        CharSequence.class, CharSequence.class, PendingIntent.class);
                m2.invoke(notifyDetails, context, context.getResources().getString(R.string
                                .noti_title),
                        text, mContentIntent);
            } catch (Exception e) {
        ...
        if (Utils.mNotificationManager != null) {
            ...
            Utils.mNotificationManager = (NotificationManager) context.getSystemService(Activity
